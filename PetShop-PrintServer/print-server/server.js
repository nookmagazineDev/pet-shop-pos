...
app.post("/print", async (req, res) => {
  try {
    const { paperWidth, shopName, shopAddress, shopPhone, shopTaxId, shopBranch, footerNote, items, subtotal, tax, total, isTest, receiptType, paymentMethod, customerInfo, empName, recNo, nonVatAdjusted, vatableAdjusted, discountAmount = 0, freeItemLines = [] } = req.body;
    ...
    printer.alignCenter();
    printer.println(isTest ? "** TEST PRINT **" : "ใบกำกับภาษี / ใบเสร็จรับเงิน");
    ...
    if (items && items.length > 0) {
      items.forEach(item => {
        ...
        if (item.vatStatus === "NON VAT") name += " (N)";
        printer.tableCustom([
      ...
      // Free items (deduction only)
      if (freeItemLines && freeItemLines.length > 0) {
        freeItemLines.forEach(fi => {
           let promoText = `Free (${fi.promoName})`;
           promoText = promoText.length > 25 ? promoText.substring(0,25) + ".." : promoText;
           printer.tableCustom([
             { text: promoText, align: "LEFT", width: 0.60 },
             { text: "", align: "CENTER", width: 0.0 },
             { text: "-" + Number(fi.price * fi.qty).toFixed(2), align: "RIGHT", width: 0.40 }
           ]);
        });
      }
      
      // Bill discount
      if (discountAmount > 0 && (!freeItemLines || freeItemLines.length === 0)) {
         printer.tableCustom([
           { text: "Discount", align: "LEFT", width: 0.60 },
           { text: "", align: "CENTER", width: 0.0 },
           { text: "-" + Number(discountAmount).toFixed(2), align: "RIGHT", width: 0.40 }
         ]);
      }
    }
