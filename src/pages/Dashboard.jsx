...
export default function Dashboard() {
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-10">
      ...
      {/* Card Detail Panel — shown ABOVE charts when a card is selected */}
      {selectedCard && (
        <div className="bg-white rounded-2xl shadow-md border border-gray-100 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-300">
          <div className="p-6 border-b border-gray-100 bg-gray-50/50 flex justify-between items-center">
            <h3 className="text-xl font-bold text-gray-900">
              {selectedCard === "sales" && "รายการออเดอร์วันนี้ (แยกตามช่องทางชำระ)"}
              {selectedCard === "orders" && "รายการออเดอร์วันนี้ (รายละเอียดสินค้า)"}
              {selectedCard === "attention" && "สินค้ารอการตรวจสอบด่วน"}
              {selectedCard === "products" && "รายการสินค้าทั้งหมดในระบบ"}
              {selectedCard === "storestock" && "สินค้าหน้าร้านที่ต้องเติม"}
            </h3>
            <button onClick={() => setSelectedCard(null)} className="text-sm font-medium text-gray-500 hover:text-gray-900 transition-colors px-3 py-1.5 rounded-lg hover:bg-gray-100">
              ปิด ✕
            </button>
          </div>
            ...
            <table className="w-full text-left text-sm border-collapse min-w-[600px]">
              {selectedCard === "sales" && (<>
                <thead className="bg-blue-50 border-b border-blue-100 text-blue-700 font-medium sticky top-0">
                  ...
                  <tr>
                    <th className="py-4 px-6">รหัสออเดอร์</th><th className="py-4 px-6">เวลา</th>
                    <th className="py-4 px-6">ช่องทางจ่าย</th><th className="py-4 px-6 text-right">จำนวนเงิน</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-100">
                  {todayTransactions.length === 0 ? (<tr><td colSpan="4" className="py-10 text-center text-gray-400">ไม่มีรายการขายวันนี้</td></tr>)
              ...
              {selectedCard === "orders" && (<>
                <thead className="bg-green-50 border-b border-green-100 text-green-700 font-medium sticky top-0">
                  <tr>
                    ...
                    <th className="py-4 px-6">รายละเอียดสินค้า</th>
                    <th className="py-4 px-6 text-right w-1/4">ยอดรวม</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-100">
                  {todayTransactions.length === 0 ? (<tr><td colSpan="3" className="py-10 text-center text-gray-400">ยังไม่มีออเดอร์วันนี้</td></tr>)
                  : todayTransactions.map((tx,idx) => {
                    let items=[]; try{items=JSON.parse(tx.CartDetails||"[]");}catch(e){}
                    return (<tr key={idx} className="hover:bg-green-50/30">
                      ...
                      <td className="py-4 px-6">
                        <ul className="space-y-2">{items.map((item,i)=>(
                          <li key={i} className="flex justify-between items-start text-sm border-b border-gray-100 pb-2 last:border-0 last:pb-0">
                            <div><div className="font-semibold text-gray-800">{item.Name||item.name}</div><div className="text-xs text-gray-400">{item.Barcode||"-"}</div></div>
                            <div className="bg-green-100 text-green-800 px-2 py-1 rounded-md font-bold text-xs shrink-0 border border-green-200">ออก {item.qty} ชิ้น</div>
                          </li>
                        ))}</ul>
                      </td>
              ...
              {selectedCard === "attention" && (<>
                <thead className="bg-red-50 border-b border-red-100 text-red-600 font-medium sticky top-0">
                  <tr>
                    <th className="py-4 px-6">ชื่อสินค้า</th><th className="py-4 px-6">รายการที่ต้องระวัง</th>
                    <th className="py-4 px-6 text-right">จำนวนเหลือ</th><th className="py-4 px-6 text-right">ราคา</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-100">
                  {attentionProducts.length === 0 ? (<tr><td colSpan="4" className="py-10 text-center text-gray-400">ไม่มีสินค้าที่ต้องกังวล 🎉</td></tr>)
                  : attentionProducts.map((p,idx)=>(
                    <tr key={idx} className="hover:bg-red-50/30">
                      <td className="py-3 px-6"><div className="font-semibold text-gray-900">{p.Name}</div><div className="text-xs text-gray-500">{p.Barcode}</div></td>
                      <td className="py-3 px-6 text-red-600 flex items-center gap-2 font-medium"><AlertTriangle size={16}/> {p.attentionReason}</td>
              ...
              {selectedCard === "products" && (<>
                <thead className="bg-purple-50 border-b border-purple-100 text-purple-700 font-medium sticky top-0">
                  <tr>
                    <th className="py-4 px-6">ชื่อสินค้า</th><th className="py-4 px-6">บาร์โค้ด</th>
                    <th className="py-4 px-6">โลเคชั่น</th><th className="py-4 px-6 text-right">จำนวนคลัง</th><th className="py-4 px-6 text-right">ราคา</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-100">
                  {allProducts.length === 0 ? (<tr><td colSpan="5" className="py-10 text-center text-gray-400">ไม่มีสินค้าในระบบ</td></tr>)
              ...
              {selectedCard === "storestock" && (<>
                <thead className="bg-emerald-50 border-b border-emerald-100 text-emerald-700 font-medium sticky top-0">
                  <tr>
                    <th className="py-4 px-6">ชื่อสินค้า</th><th className="py-4 px-6">ตำแหน่งหน้าร้าน</th>
                    <th className="py-4 px-6">อัปเดตล่าสุด</th><th className="py-4 px-6 text-right">คงเหลือ</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-100">
                  {lowStoreStock.length === 0 ? (<tr><td colSpan="4" className="py-10 text-center text-gray-400">สินค้าหน้าร้านเพียงพอ 🎉</td></tr>)
                  : lowStoreStock.map((s,idx)=>(
                    <tr key={idx} className="hover:bg-emerald-50/30">
                      <td className="py-3 px-6"><div className="font-semibold text-gray-900">{s.Name}</div><div className="text-xs text-gray-500 font-mono">{s.Barcode}</div></td>
                      ...
                      <td className="py-3 px-6 text-right"><span className="font-bold text-lg text-red-600">{s.Quantity}</span><div className="text-xs text-red-500 font-medium">ต้องเติมด่วน!</div></td>
                    </tr>
                  ))}
                </tbody>
              </>)}
      ...
      {/* Charts & Activity — always visible below */}
      <div className="space-y-6">
                    ...
                    content={({ active, payload, label }) => {
                      ...
                      return (
                        <div style={{ background: '#fff', borderRadius: 12, boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.15)', padding: '12px 16px', minWidth: 160 }}>
                          ...
                          <p style={{ color: '#3B82F6', fontWeight: 600 }}>ยอดขายรวม: ฿{(payload[0].value || 0).toLocaleString()}</p>
                        </div>
                      );
                    }}
                  />
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 min-h-[420px] flex flex-col">
          <h3 className="text-lg font-bold text-gray-900 mb-3">ช่องทางชำระเงิน</h3>

          {/* Payment Breakdown Summary */}
          {chartDataPayment.length > 0 && !isLoading && (() => {
            const total = chartDataPayment.reduce((s, d) => s + d.value, 0);
            return (
                ...
                {chartDataPayment.map((item, index) => (
                  <div key={index} className="flex items-center justify-between bg-gray-50 rounded-lg px-3 py-2">
                    ...
                    <div className="text-right">
                      <span className="text-sm font-bold text-gray-900">฿{item.value.toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
                      <span className="text-xs text-gray-400 ml-2">({total > 0 ? ((item.value / total) * 100).toFixed(1) : 0}%)</span>
                    </div>
                  </div>
                ))}
              </div>
                ...
                <PieChart>
                  ...
                  <Tooltip
                    content={({ active, payload }) => {
                      ...
                      return (
                        <div style={{ background: '#fff', borderRadius: 12, boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.15)', padding: '12px 16px', minWidth: 180 }}>
                          <p style={{ fontWeight: 700, color: '#111827', marginBottom: 6 }}>{entry.name}</p>
                          <p style={{ color: entry.payload.fill || '#6B7280', fontWeight: 600 }}>฿{entry.value.toLocaleString()}</p>
                          <p style={{ color: '#6B7280', fontSize: 12, marginTop: 4 }}>สัดส่วน: {pct}%</p>
                        </div>
                      );
                    }}
                  />
                ...
                <BarChart data={chartDataCategory} margin={{ top: 5, right: 0, left: 0, bottom: 5 }}>
                  ...
                  <Tooltip
                    content={({ active, payload, label }) => {
                      ...
                      return (
                        <div style={{ background: '#fff', borderRadius: 12, boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.15)', padding: '12px 16px', minWidth: 180 }}>
                          <p style={{ fontWeight: 700, color: '#111827', marginBottom: 6 }}>หมวด: {label}</p>
                          <p style={{ color: entry.fill || '#8B5CF6', fontWeight: 600 }}>ยอดขาย: ฿{(entry.value || 0).toLocaleString()}</p>
                        </div>
                      );
                    }}
                    cursor={{ fill: '#F3F4F6' }}
