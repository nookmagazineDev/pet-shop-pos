...
export default function TaxInvoiceModal({ isOpen, onClose, cart, paymentMethod, subtotal, discountAmount = 0, freeItemLines = [], tax, total, receiptType, customerInfo, taxInvoiceNo }) {
  ...
  // ราคา = VAT inclusive → ถอด VAT ออก: discount ที่ proportionate
  ...
  // vatableAdjusted = ราคา VAT items หลังหักส่วนลด (ยังรวม VAT 7% อยู่)
  const vatableIncl = total - nonVatAdjusted;
  ...
  const handlePrint = async () => {
    ...
    const rows = cart.map(item => {
      ...
      return `<tr>
        <td style="word-break: break-word; padding-right: 4px; font-size: 0.95em;">${item.name || item.Name}${item.vatStatus === 'NON VAT' ? ' (N)' : ''}</td>
    ...
    // Bill discount row
    const billDiscountRow = discountAmount > 0 && freeItemLines.length === 0 ? `
      <tr style="color:#9333ea; font-weight:bold;">
        <td colspan="3">ส่วนลดท้ายบิล</td>
        <td style="text-align:right">-${discountAmount.toLocaleString("th-TH", { minimumFractionDigits: 2 })}</td>
      </tr>
    ` : "";
        ...
        <div className="flex-1 overflow-auto p-8 print:p-0">
          ...
          <table className="w-full text-left border-collapse mb-6">
            ...
            <tbody>
              {cart.map((item, i) => (
                <tr key={i} className="border-b border-gray-100">
                  <td className="py-3 text-sm">
                    ...
                    {item.vatStatus === "NON VAT" && <span className="ml-1 text-xs text-gray-400 border border-gray-300 rounded px-1">(N)</span>}
                    <br/><span className="text-xs text-gray-400">{item.barcode || item.Barcode}</span>
                  </td>
              ...
              {/* Free item lines (only deduction) */}
              ...
              {/* Bill-level discount (non-free-item) */}
              {discountAmount > 0 && freeItemLines.length === 0 && (
